...
::ghep anh thanh video
	ffmpeg -framerate 0.2 -i %dir_manga%/%%03d.jpg video_raw.avi
::Add music vao video
	ffmpeg -y -i music.mp3 -i video_raw.avi -codec copy -shortest -preset superfast video_music.avi
::chen logo
	ffmpeg -i video_music.avi -i logo_small.png -filter_complex "overlay=0:0" %dir_manga%/%vid_name%.avi	
) ELSE (
::ghep anh thanh video
	ffmpeg -framerate 0.2 -i %dir_manga%/%%03d.jpg -vf format=yuv420p video_raw.mp4
::Add music vao video
	ffmpeg -y -i music.mp3 -i video_raw.mp4 -codec copy -shortest -preset superfast video_music.mp4
::chen logo	
	ffmpeg -i video_music.mp4 -i logo.png -filter_complex "overlay=0:0" %dir_manga%/%vid_name%.mp4
)
del video_raw.avi
del video_raw.mp4
del video_music.avi
